...
diagnostico='epicondilitis' # Modificar diagnóstico
...
for i in range(n_tareas): # Se activa for/loop por cada tarea
    ...
    if ext_muneca[1]<=25:
        resultado=resultado+0 # Si extensión es menor a 25°, el puntaje será 0
    elif (ext_muneca[1]>25) & (ext_muneca[1]<=40):
        resultado=resultado+1 # Si extensión esta en el intervalo ]25°,40°], el puntaje será 1
    elif ext_muneca[1]>40:
        resultado=resultado+2 # Si extensión es mayor a 40°, el puntaje será 2
    ...
    if supinacion=='Ausente':
        resultado=resultado+0 # Si la supinación esta ausente, el puntaje será 0
    elif supinacion=='Rango Intermedio':
        resultado=resultado+1 # Si la supinación esta en rango intermedio, el puntaje será 1
    elif supinacion=='Rango Extremo':
        resultado=resultado+2 # Si la supinación esta en rango extremo, el puntaje será 2
    ...
    elif ((resultado>=1) & (resultado<=2)) | (ext_muneca[1]<=25): # Si muñeca extendida <26°, la tarea tendrá riesgo insuficiente, independientemente del puntaje obtenido
        nivel_riesgo='Insuficiente por puntaje o porque la extensión de muñeca es ≤ 25°'
        color='Grey'
...
if guardar==True: # Si se da click al botón, entonces se guardaran los datos en una planilla excel
    ...
    # token='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'
    # g = Github(token)

    # # Get the repository
    # repo = g.get_repo("lalarconm/App-EP-ME")
    # contents = repo.get_contents(file_path)
    # # Create a new file in the repository with the data.xlsx file
    # # with open(file_path, "rb") as file:
    # #     content = file.read()
    # #     repo.create_file(file_path, "Auto save", content)

    # # Update the existing file with the new data
    # with open(file_path, "rb") as file:
    #     content = base64.b64encode(file.read())
    #     repo.update_file(contents.path, "Actualizar archivo Excel", content.decode(), contents.sha)
    ...
    folder.upload_file(fileContent, file_path)
    file.close()
    'Registro cargado'
